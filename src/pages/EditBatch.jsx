...
export default function EditBatch() {
  ...
  const { id } = useParams(); // Grab the batch ID from the URL
  ...
  // 2. FETCH EXISTING BATCH DATA
  useEffect(() => {
    const fetchBatch = async () => {
      try {
        const responseData = await recyclingService.getBatchById(id);
        const batch = responseData?.data?.result || responseData?.data || responseData;
        ...
        setFormData({
          ...
          total_weight_kg: batch.total_weight || batch.total_weight_kg || '',
          weight_unit: batch.weight_unit || 'kg',
          est_price: batch.price_per_unit || batch.est_price || '', // Backend might send price_per_unit instead
        ...
        navigate('/recycling/inventory'); // Kick back if it fails
      }
    };
  ...
  const handleSubmit = async () => {
    if (!formData.title || !formData.batch_type || !formData.total_weight_kg) {
    ...
    try {
      ...
      // Laravel Quirk: Must use POST and spoof PUT when uploading files
      payload.append('_method', 'PUT');

      await recyclingService.updateBatch(id, payload);

      toast.success("Batch updated successfully!", { id: toastId });
      navigate('/recycling/inventory'); 
      ...
      if (error.response?.status === 422) {
        ...
        toast.error(error.response?.data?.message || "Server error. Could not update batch.", { id: toastId });
      }
      ...
      <main className="max-w-5xl w-full mx-auto px-6 py-8 lg:px-12 animate-in fade-in duration-300">
        ...
        <div className="mb-12 mt-4">
          <nav className="flex items-center gap-2 text-slate-500 text-sm mb-4 font-body">
            ...
            <span className="text-white font-medium">Edit Batch</span>
          </nav>
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
            <div>
              <h2 className="text-4xl font-headline font-bold text-white tracking-tight mb-2">Edit Batch</h2>
              <p className="text-slate-400 font-body text-sm">Update material parameters and manifest documentation.</p>
            </div>
              ...
              <div className="space-y-6">
                ...
                <div>
                  ...
                  <div className="flex items-end gap-4">
                    <input name="total_weight_kg" value={formData.total_weight_kg} onChange={handleChange} className="flex-1 bg-[#2e3543] border-b border-slate-600 text-white font-body py-3 px-4 focus:outline-none focus:border-b-primary focus:border-b-2 transition-all rounded-t-lg outline-none" type="number"/>
              ...
              <div className="space-y-6">
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input name="est_price" value={formData.est_price} onChange={handleChange} className="w-full bg-[#2e3543] border-b border-slate-600 text-white font-body py-3 pl-8 pr-4 focus:outline-none focus:border-b-primary focus:border-b-2 transition-all rounded-t-lg outline-none" type="number"/>
                  </div>
                </div>
              </div>
            </div>
                ...
                <label className="mt-2 border-2 border-dashed border-slate-600 rounded-xl bg-[#2e3543]/50 p-12 flex flex-col items-center justify-center hover:bg-[#2e3543] hover:border-primary/50 transition-all cursor-pointer group relative overflow-hidden">
                  ...
                  {imagePreview ? (
                    ...
                    <>
                      ...
                      <p className="text-white font-bold font-body mb-1">Click to upload new image</p>
